...
class myLSTM:
    ...
    def _cal_gate(self, wh, wx, b, ht_pre, x, activation):
        #return activation(wh.dot(ht_pre) + wx[:, x].reshape(-1, 1) + b) # wrong
        return activation(wh.dot(ht_pre) + wx.dot(x.reshape(-1,1)) + b)
...
def main():
    ...
    x_train, y_train= data_generator(10000)

    lstm = myLSTM(2,16)
    lstm.train(x_train, y_train, 0.1, 5)
    x, y = data_generator(100)
    #lstm._predict(x, y)
    #print(x)
    for p in range(100):
        y_pre = lstm.predict(x[p])
        print("Pred:" + str(y_pre))
        print("True:" + str(y[p]))
        a_int=0
        for index, o in enumerate(x[p].T[0]):
            a_int += o * pow(2, index)
        b_int=0
        for index, o in enumerate(x[p].T[1]):
            b_int += o * pow(2, index)
        out = 0
        for index, o in enumerate(y_pre):
            out += o * pow(2, index)
